...
function Movex({ id, isDraggable = true }) {
  const { stepInXValues, setStepInX } = useContext(Getcontext);
  const initialStep = stepInXValues && stepInXValues[id] !== undefined ? stepInXValues[id] : 0;
  const [step, setStep] = useState(initialStep);

  useEffect(() => {
    const contextStep = stepInXValues && stepInXValues[id] !== undefined ? stepInXValues[id] : 0;
    setStep(contextStep);
  }, [stepInXValues, id]);
  ...
  return (
    ...
    >
      <span className="truncate">Move X &nbsp;</span>
      <input
        type="number"
